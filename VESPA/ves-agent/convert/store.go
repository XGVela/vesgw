...
func (fm *FaultManager) storeFault(faultName string) (StatusResult, int32) {
	...
	if val != 0 {
		logging.LogForwarder("WARNING", syscall.Gettid(), "convert", "storeFault", fmt.Sprintf("fault name %s already exist with id %d", faultName, val))
		id = val
		status = AlreadyExist
		...
		if err = fm.state.StoreFaultInStorage(faultName, faultID); err != nil {
			logging.LogForwarder("ERROR", syscall.Gettid(), "convert", "storeFault", err.Error())

			return InError, 0
		}
		logging.LogForwarder("INFO", syscall.Gettid(), "convert", "storeFault", fmt.Sprintf("store fault: %s with index %d \n", faultName, faultID))
		id = faultID
		status = Stored
...
func (fm *FaultManager) clearFault(faultName string) (StatusResult, int32) {
	...
	if id == 0 {
		logging.LogForwarder("WARNING", syscall.Gettid(), "convert", "clearFault", "clearFault: fault name not present in storage: "+faultName)
		status = NotExist
	} else {
		//fm.state.DeleteFaultInStorage(faultName)
		logging.LogForwarder("INFO", syscall.Gettid(), "convert", "clearFault", fmt.Sprintf("suppress fault name %s with id %d in storage \n ", faultName, id))
