...
func mustNotCall() error {
	logging.LogForwarder("EXCEPTION", syscall.Gettid(), "convert", "mustNotCall", "Commit function should not have been called")
...
func AlertToFault(alert template.Alert, fm *FaultManager, namingCodes map[string]string) (StatusResult, *govel.EventFault, CommitFunc) {
	...
	logging.LogForwarder("DEBUG", syscall.Gettid(), "convert", "AlertToFault", "convert alert to VES event fault: ", label["alertname"])
	...
	if !exist {
		logging.LogForwarder("ERROR", syscall.Gettid(), "convert", "AlertToFault", "Error in severityToPriority convert for severity : "+label["severity"])
	...
	logging.LogForwarder("DEBUG", syscall.Gettid(), "convert", "AlertToFault", "sourceName", sourceName)
	...
	if storeStatus == Stored {
		if err := fm.GetFaultState().SetFaultStartEpoch(id, eventFault.StartEpochMicrosec); err != nil {
			logging.LogForwarder("ERROR", syscall.Gettid(), "convert", "AlertToFault", err.Error())
	...
	logging.LogForwarder("DEBUG", syscall.Gettid(), "convert", "AlertToFault", fmt.Sprintf("AlertToFault success for id %s sequence %d: \n", vesID, eventFault.Sequence))

	var commitFunc CommitFunc
	if storeStatus == Cleared {
		commitFunc = func() error {
			logging.LogForwarder("DEBUG", syscall.Gettid(), "convert", "AlertToFault", fmt.Sprintf("Delete Fault %s with id %d in storage \n ", faultName, id))
...
func GetFaultID(vesID string) int32 {
	...
	if err != nil {
		logging.LogForwarder("WARNING", syscall.Gettid(), "convert", "GetFaultID", "problem to extract id from vesID")
...
func buildAdditionalInfos(service string, aaiMapping string) []govel.EventField {
	...
	if len(alaInfos) == 0 || len(alaInfos) != len(eventNames)+1 {
		logging.LogForwarder("WARNING", syscall.Gettid(), "convert", "buildAdditionalInfos", "incorrect information in alert service field")

	} else {
		for i := 1; i < len(alaInfos); i++ {
			events = append(events, govel.EventField{})
		}
	}
	return events
}
