...
func (keys MeasKeys) String() string {
	...
	for k, v := range keys {
		if _, err := fmt.Fprintf(&bld, "%s: %s,", k, v); err != nil {
			logging.LogForwarder("EXCEPTION", syscall.Gettid(), "metrics", "String", err.Error())
...
func (bld *EventMeasurementSetBuilder) checkValid() {
	if !bld.valid {
		logging.LogForwarder("EXCEPTION", syscall.Gettid(), "metrics", "generateUUID", "EventMeasurementSetBuilder has an illegal state")
	}
}
