...
func measTestFactory(interval time.Duration) MeasurementFactory {
	return MeasurementFactoryFunc(func(vmID string, timestamp time.Time) (*govel.EventMeasurements, error) {
		return govel.NewMeasurements("name", "id", vmID, interval, timestamp.Add(-interval), timestamp), nil
	})
}
...
func TestMeasurementSetToBatch(t *testing.T) {
	meas := EventMeasurementSet{
		govel.NewMeasurements("meas1", "id1", "source1", 10*time.Second, time.Now(), time.Now()),
		govel.NewMeasurements("meas2", "id2", "source2", 10*time.Second, time.Now(), time.Now()),
	}
	batch := meas.Batch()
