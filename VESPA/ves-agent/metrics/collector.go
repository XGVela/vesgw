...
type Collector struct {
	...
	evtCfg    *govel.EventConfigurationWithoutNfc // Generals event configuration
...
func NewCollectorWithState(cfg *config.MeasurementConfiguration, evtCfg *govel.EventConfigurationWithoutNfc, state CollectorState) (*Collector, error) {
	logging.LogForwarder("INFO", syscall.Gettid(), "metrics", "NewCollectorWithState", "Initializing Prometheus Measurement Collector to "+fmt.Sprintf("%v", cfg.Prometheus.Address))
...
func NewCollector(cfg *config.MeasurementConfiguration, evtCfg *govel.EventConfigurationWithoutNfc) (*Collector, error) {
	return NewCollectorWithState(cfg, evtCfg, &inMemState{index: 0})
}
...
func (col *Collector) adjustCollectionStartTime(start time.Time, end time.Time, interval time.Duration) time.Time {
	if col.max > 0 && end.Sub(start) > col.max {
		logging.LogForwarder("DEBUG", syscall.Gettid(), "metrics", "adjustCollectionStartTime", "Rounding collect timeframe to "+fmt.Sprintf("%s", col.max.String()))
...
func (col *Collector) CollectMetrics(from, to time.Time, interval time.Duration) (EventMeasurementSet, error) {
	...
	evtName := col.domainAbr + "_" + nfNamingCode + "-" + col.evtCfg.Tmaas.VendorID + "_Measurements"
	metrics := NewEventMeasurementSetBuilder(MeasurementFactoryFunc(func(vmID string, timestamp time.Time) (*govel.EventMeasurements, error) {
		...
		mvmid := strings.Split(vmID, "_")
		procMicroSvc := mvmid[1]
		fmt.Println("vmID======>>>>>>>>>>>>", vmID, " procMicroSvc====>>>>>>>>>>> ", procMicroSvc)
		//meas.NfcNamingCode = col.metriccfg.Measurement.Prometheus.Rules[i].NfcNamingCodes
		meas.NfcNamingCode = procMicroSvc
		//meas.NfcNamingCode = col.evtCfg.NfcNamingCodes
		//meas.EventType = "?????????????"           // Change PRD - name sapce
		// meas.EventType = col.evtCfg.VNFName
		meas.EventType = "MEASUREMENT"
		//meas.ReportingEntityID = "???????????????" // Change PRD
		meas.ReportingEntityID = col.evtCfg.ReportingEntityID
		//meas.ReportingEntityName = meas.EventType  // Change PRD
		meas.ReportingEntityName = config.DefaultConf.Heartbeat.AdditionalFields.DnPrefix + col.evtCfg.ReportingEntityName
		meas.SourceName = config.DefaultConf.Heartbeat.AdditionalFields.DnPrefix + col.evtCfg.SourceName
		//meas.SourceID = meas.ReportingEntityID     // Change PRD
		//meas.SourceID = col.evtCfg.ReportingEntityID
		meas.SourceID = generateUUID(meas.SourceName)
		meas.NfVendorName = col.evtCfg.Tmaas.VendorID
		//meas.SourceName = meas.ReportingEntityName // Change PRD
		//meas.SourceName = col.evtCfg.ReportingEntityName
		return meas, nil
	}))
	...
	logging.LogForwarder("INFO", syscall.Gettid(), "metrics", "adjustCollectionStartTime", "Starting metrics collection")
	...
	for _, rules := range col.rules {

		for _, rule := range rules.Metrics {

			// Iterate over rules, query prometheus, convert and collect results
			// into the measurement set builder
			if err := col.collectFromRule(&metrics, rule.WithDefaults(rules.Defaults), rng); err != nil {
				return nil, err
			}
		}
	}
	logging.LogForwarder("INFO", syscall.Gettid(), "metrics", "adjustCollectionStartTime", "Metrics collection completed in "+fmt.Sprintf("%s", time.Since(start)))
...
func (col *Collector) collectFromRule(metrics *EventMeasurementSetBuilder, rule config.MetricRule, rng v1.Range) error {
	...
	mvmid := strings.Split(rule.VMIDLabel, "_")
	procNameSpace := mvmid[0]
	procMicroSvc := mvmid[1]
	logging.LogForwarder("TRACER", syscall.Gettid(), "metrics", "collectFromRule", "Inside collectFromRule - "+fmt.Sprintf("%s", rule.VMIDLabel)+fmt.Sprintf("%s", procNameSpace)+fmt.Sprintf("%s", procMicroSvc))
	...
	logging.LogForwarder("TRACER", syscall.Gettid(), "metrics", "collectFromRule", fmt.Sprintf("Resulted range is%v ", res))

	for _, meas := range res {
		labels := map[string]string{}
		isNotProcNameSpace := false
		isNotProcMicroService := false
		for lab, val := range meas.Metric {
			labels[string(lab)] = string(val)

			logging.LogForwarder("TRACER", syscall.Gettid(), "metrics", "collectFromRule", fmt.Sprintf("Labels are : %v %v", lab, val))

			if string(lab) == "microSvcName" {
				if string(val) == procMicroSvc {
					logging.LogForwarder("TRACER", syscall.Gettid(), "metrics", "collectFromRule", "Micro Service name Matched")
				} else {
					logging.LogForwarder("TRACER", syscall.Gettid(), "metrics", "collectFromRule", "Micro Service name Not Matched")
					isNotProcMicroService = true
				}
			}
			if string(lab) == "kubernetes_namespace" {
				if string(val) == procNameSpace {
					logging.LogForwarder("TRACER", syscall.Gettid(), "metrics", "collectFromRule", "kubernetes_namespace Matched")
				} else {
					logging.LogForwarder("TRACER", syscall.Gettid(), "metrics", "collectFromRule", "kubernetes_namespace not Matched")
					isNotProcNameSpace = true
				}
			}

		}
		if (isNotProcMicroService) || (isNotProcNameSpace) {
			continue
		}
		...
		if err != nil {
			logging.LogForwarder("ERROR", syscall.Gettid(), "metrics", "collectFromRule", "Cannot evaluate vmID: "+err.Error())
		...
		if err != nil || target == "" {
			...
			logging.LogForwarder("WARNING", syscall.Gettid(), "metrics", "collectFromRule", "Cannot evaluate target: "+err.Error())
		...
		keys := MeasKeys{}
		lbls := rule.Labels
		logging.LogForwarder("INFO", syscall.Gettid(), "metrics", "collectFromRule", "Obtained Target is:"+fmt.Sprintf("%s", target))
		logging.LogForwarder("INFO", syscall.Gettid(), "metrics", "collectFromRule", "rule.ObjectKeys====>"+fmt.Sprintf("%v", rule.ObjectKeys))
		...
		for _, val := range meas.Values {
			...
			logging.LogForwarder("INFO", syscall.Gettid(), "metrics", "collectFromRule", fmt.Sprintf("Got metric %s{%s} => time: %s, VNFC: %s, value: %s", fmt.Sprintf("%s", target), fmt.Sprintf("%s", keys.String()), fmt.Sprintf("%s", timestamp.String()), fmt.Sprintf("%s", vnfc), fmt.Sprintf("%s", val.Value)))

			var err error
			if target == "AdditionalObjects" {
				logging.LogForwarder("ERROR", syscall.Gettid(), "metrics", "collectFromRule", "calling metrics.SetAdditionalObject")
				err = metrics.SetAdditionalObject(vnfc, rule.ObjectName, rule.ObjectInstance, timestamp, float64(val.Value), keys)
			} else {
				logging.LogForwarder("INFO", syscall.Gettid(), "metrics", "collectFromRule", "calling metrics.Set")
	...
	logging.LogForwarder("TRACER", syscall.Gettid(), "metrics", "collectFromRule", "Leaving collectFromRule - ", rule.VMIDLabel)
	return nil
}

func (col *Collector) getMatrix(query string, r v1.Range) (model.Matrix, error) {
	logging.LogForwarder("DEBUG", syscall.Gettid(), "metrics", "getMatrix", "Prometheus query : "+fmt.Sprintf("%s", query))
...
func generateUUID(reportingEntityName string) string {
	...
	logging.LogForwarder("DEBUG", syscall.Gettid(), "metrics", "generateUUID", "uuid generated is:"+fmt.Sprintf("%s", newId))
