...
func NewSchedulerWithState(name string, job Job, defaultInterval time.Duration, state State) *Scheduler {
	if job == nil {
		logging.LogForwarder("EXCEPTION", syscall.Gettid(), "scheduler", "NewSchedulerWithState", "job cannot be null")
	}
	logging.LogForwarder("INFO", syscall.Gettid(), "scheduler", "NewSchedulerWithState", "Creating Scheduler "+fmt.Sprintf("%s", name)+"default interval: "+fmt.Sprintf("%s", defaultInterval.String()))
...
func (sched *Scheduler) SetInterval(interval time.Duration) error {
	...
	logging.LogForwarder("INFO", syscall.Gettid(), "scheduler", "SetInterval", "Scheduler %s: interval set to "+fmt.Sprintf("%s", sched.name)+fmt.Sprintf("%v", newInterval))

	logging.LogForwarder("INFO", syscall.Gettid(), "scheduler", "SetInterval", "Scheduler %s: next run at "+fmt.Sprintf("%s", sched.name)+fmt.Sprintf("%s", sched.NextRun().String()))
	return nil
}
...
func (sched *Scheduler) NextRun() time.Time {
	...
	if nextTime.IsZero() {
		...
		if err := sched.state.UpdateNextRun(sched.name, nextTime); err != nil {
			...
			logging.LogForwarder("WARNING", syscall.Gettid(), "scheduler", "NextRun", "Scheduler"+fmt.Sprintf("%s", sched.name)+":"+"Cannot update next run time: "+err.Error())
		}
		logging.LogForwarder("INFO", syscall.Gettid(), "scheduler", "NextRun", "Scheduler :"+fmt.Sprintf("%s", sched.name)+"next run at :"+fmt.Sprintf("%s", nextTime.String())+"in "+fmt.Sprintf("%s", time.Until(nextTime).String()))
...
func (sched *Scheduler) WaitChan() *time.Timer {
	...
	logging.LogForwarder("DEBUG", syscall.Gettid(), "scheduler", "WaitChan", "Scheduler "+fmt.Sprintf("%s", sched.name)+" : waiting for "+fmt.Sprintf("%s", sleepTime.String()))
...
func (sched *Scheduler) Ack() error {
	...
	logging.LogForwarder("INFO", syscall.Gettid(), "scheduler", "Ack", "Scheduler "+fmt.Sprintf("%s", sched.name)+" Ack: Updating next run time from"+" to "+fmt.Sprintf("%s", sched.NextRun().String())+fmt.Sprintf("%s", next.String()))
	if err := sched.state.UpdateNextRun(sched.name, next); err != nil {
		return err
	}
	sched.lastTime = nil
	logging.LogForwarder("INFO", syscall.Gettid(), "scheduler", "Ack", "Scheduler "+fmt.Sprintf("%s", sched.name)+" : Next run in "+fmt.Sprintf("%v", time.Until(sched.NextRun())))
	return nil
}
