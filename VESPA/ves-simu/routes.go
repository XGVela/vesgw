...
func initRoutes() *mux.Router {
	...
	router.Methods(http.MethodPost).
		Path(*serverRoot+"/eventListener/v7"+*topic).
		// Path(*serverRoot+*topic).
		Headers("Content-Type", "application/json").
		Handler(errorWrapper(handlePostEvent))

	router.Methods(http.MethodPost).
		Path(*serverRoot+"/eventListener/v7/eventBatch").
		// Path(*serverRoot).
		Headers("Content-Type", "application/json").
		Handler(errorWrapper(handlePostBatch))

	router.Methods(http.MethodGet).
		Path("/testControl/v7/events").
	...
	router.Methods(http.MethodPost).
		Path("/testControl/v7/commandList").
		Headers("Content-Type", "application/json").
