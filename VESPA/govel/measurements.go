...
// NicPerformance describes the performance and errors of an identified virtual network interface card
type NicPerformance struct {
	...
	ReceivedPercentDiscard                 *float64         `json:"receivedPercentDiscard,omitempty"`                 // Percentage of discarded packets received; value = (receivedDiscardedPacketsDelta / receivedTotalPacketsDelta) x 100, if denominator is nonzero, or 0, if otherwise
	ReceivedPercentError                   *float64         `json:"receivedPercentError,omitempty"`                   // Percentage of error packets received; value = (receivedErrorPacketsDelta / receivedTotalPacketsDelta) x 100, if denominator is nonzero, or 0, if otherwise.
	ReceivedUtilization                    *float64         `json:"receivedUtilization,omitempty"`                    // Percentage of utilization received; value = (receivedOctetsDelta / (speed x (lastEpochMicrosec - startEpochMicrosec))) x 100, if denominator is nonzero, or 0, if otherwise
	Speed                                  *float64         `json:"speed,omitempty"`                                  // Speed configured in mbps
	TransmittedPercentDiscard              *float64         `json:"transmittedPercentDiscard,omitempty"`              // Percentage of discarded packets transmitted; value = (transmittedDiscardedPacketsDelta / transmittedTotalPacketsDelta) x 100, if denominator is nonzero, or 0, if otherwise
	TansmittedUtilization                  *float64         `json:"transmittedUtilization,omitempty"`                 // Percentage of utilization transmitted; value = (transmittedOctetsDelta / (speed x (lastEpochMicrosec - startEpochMicrosec))) x 100, if denominator is nonzero, or 0, if otherwise.
...
type MemoryUsage struct {
	...
	MemoryLatencyAvg     *float64 `json:"memoryLatencyAvg,omitempty"`     // Percentage of time the VM is waiting to access swapped or compressed memory
	MemorySharedAvg      *float64 `json:"memorySharedAvg,omitempty"`      // shared memory in kilobytes
	MemorySwapInAvg      *float64 `json:"memorySwapInAvg,omitempty"`      // Amount of memory swapped-in from host cache in kibibytes
	MemorySwapInRateAvg  *float64 `json:"memorySwapInRateAvg,omitempty"`  // rate at which memory is swapped from disk into active memory during the interval in kilobytes per second
	MemorySwapOutAvg     *float64 `json:"memorySwapOutAvg,omitempty"`     // Amount of memory swapped-out to host cache in kibibytes
	MemorySwapOutRateAvg *float64 `json:"memorySwapOutRateAvg,omitempty"` // rate at which memory is being swapped from active memory to disk during the current interval in kilobytes per second
	MemorySwapUsedAvg    *float64 `json:"memorySwapUsedAvg,omitempty"`    // space used for caching swapped pages in the host cache in kibibytes
	PercentMemoryUsage   *float64 `json:"percentMemoryUsage,omitempty"`   // Percentage of memory usage; value = (memoryUsed / (memoryUsed + memoryFree) x 100 if denomintor is nonzero, or 0, if otherwise
...
type DiskUsage struct {
	...
	DiskBusResets             *float64 `json:"diskBusResets,omitempty"`             // number of bus resets over the measurementInterval
	DiskCommandsAborted       *float64 `json:"diskCommandsAborted,omitempty"`       // number of disk commands aborted over the measurementInterval
	DiskCommandsAvg           *float64 `json:"diskCommandsAvg,omitempty"`           // average number of commands per second over the measurementInterval
	DiskFlushRequests         *float64 `json:"diskFlushRequests,omitempty"`         // total flush requests of the disk cache over the measurementInterval
	DiskFlushTime             *float64 `json:"diskFlushTime,omitempty"`             // milliseconds spent on disk cache flushing over the measurementInterval
	DiskReadCommandsAvg       *float64 `json:"diskReadCommandsAvg,omitempty"`       // average number of read commands issued per second to the disk over the measurementInterval
	DiskTime                  *float64 `json:"diskTime,omitempty"`                  // nanoseconds spent on disk cache reads/writes within the measurement interval
	DiskTotalReadLatencyAvg   *float64 `json:"diskTotalReadLatencyAvg,omitempty"`   // average read time from the perspective of a Guest OS: sum of the Kernel Read Latency and Physical Device Read Latency in milliseconds over the measurement interval
	DiskTotalWriteLatencyAvg  *float64 `json:"diskTotalWriteLatencyAvg,omitempty"`  // average write time from the perspective of a Guest OS: sum of the Kernel Write Latency and Physical Device Write Latency in milliseconds over the measurement interval
	DiskWeightedIoTimeAvg     *float64 `json:"diskWeightedIoTimeAvg,omitempty"`     // measure in ms over 1 sec of both I/O completion time and the backlog that may be accumulating; value is the average within the collection interval
	DiskWeightedIoTimeLast    *float64 `json:"diskWeightedIoTimeLast,omitempty"`    // measure in ms over 1 sec of both I/O completion time and the backlog that may be accumulating; value is the last within the collection interval
	DiskWeightedIoTimeMax     *float64 `json:"diskWeightedIoTimeMax,omitempty"`     // measure in ms over 1 sec of both I/O completion time and the backlog that may be accumulating; value is the maximum within the collection interval
	DiskWeightedIoTimeMin     *float64 `json:"diskWeightedIoTimeMin,omitempty"`     // measure in ms over 1 sec of both I/O completion time and the backlog that may be accumulating; value is the minimum within the collection interval
	DiskWriteCommandsAvg      *float64 `json:"diskWriteCommandsAvg,omitempty"`      // average number of write commands issued per second to the disk over the measurementInterval
...
type CPUUsage struct {
	...
	CPUCapacityContention int      `json:"cpuCapacityContention,omitempty"`
	...
	CPUDemandAvg          *float64 `json:"cpuDemandAvg,omitempty"`      // the total CPU time that the NF/NFC/VM could use if there was no contention, in milliseconds over the measurementInterval
	CPUDemandMhz          *float64 `json:"cpuDemandMhz,omitempty"`      // CPU demand in megahertz
	CPUDemandPct          *float64 `json:"cpuDemandPct,omitempty"`      // CPU demand as a percentage of the provisioned capacity
	CPULatencyAvg         *float64 `json:"cpuLatencyAvg,omitempty"`     // percentage of time the VM is unable to run because it is contending for access to the physical CPUs
	CPUOverheadAvg        *float64 `json:"cpuOverheadAvg,omitempty"`    // the overhead demand above available allocations and reservations, in milliseconds over the measurementInterval
	CPUSwapWaitTime       *float64 `json:"cpuSwapWaitTime,omitempty"`   // swap wait time. in milliseconds over the measurementInterval
}
...
// MeasurementFields name value pair
type MeasurementFields struct {
	...
	//HugePagesArray           []hugePages            `json:"hugePagesArray,omitempty"`   //array of metrics on hugepPages
	//Impi                     //Need to define        //Need to define                   //Need to define
	...
	//LoadArray              []load             `json:"loadArray",omitempty"`             //array of system load metrics
	//MachineCheckExceptionArray             []machineCheckException             `json:"machineCheckExceptionArray",omitempty"`             //array of machine check exceptions
	...
	MeasurementFieldsVersion string           `json:"measurementFieldsVersion"`          // version of the measurementsForVfScaling block
	...
	NfcScalingMetric         *int64           `json:"nfcScalingMetric,omitempty"`        // represents busy-ness of the VNF from 0 to 100 as reported by the VNFC
	NicPerformanceArray      []NicPerformance `json:"nicPerformanceArray,omitempty"`     // usage of an array of virtual network interface cards
	//ProcessStatsArray        []processStats   `json:"processStatsArray,omitempty"`       //array of metrics on system processes
}

// EventMeasurements is a metric event
type EventMeasurements struct {
	EventHeader       `json:"commonEventHeader"`
	MeasurementFields `json:"measurementFields"`
}
...
func NewMeasurements(name, id string, sourceName string, interval time.Duration, start, end time.Time) *EventMeasurements {
	...
	meas.MeasurementFieldsVersion = "4.0"
	meas.VesEventListenerVersion = "7.1"
	meas.MeasurementInterval = interval.Seconds()

	meas.Domain = DomainMeasurements
	//meas.SourceName = sourceName
	meas.EventName = name // Change PRD
	meas.EventID = id
	meas.Version = "4.1"
	meas.Priority = PriorityNormal

	//meas.EventType = "?????????????"           // Change PRD - name sapce
	//meas.ReportingEntityID = "???????????????" // Change PRD
	//meas.ReportingEntityName = meas.EventType  // Change PRD
	//meas.SourceID = meas.ReportingEntityID     // Change PRD
	//meas.SourceName = meas.ReportingEntityName // Change PRD
