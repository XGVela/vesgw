...
func NewEvel(collector *CollectorConfiguration, event *EventConfiguration, cacert string) (*Evel, error) {
	...
	path = strings.TrimRight(path, "/") + "/eventListener/v7"
	baseURL := url.URL{
		Scheme: httpScheme,
	...
	client := NewVESClient(baseURL, &tlsConfig, schema.V3011(), event.MaxSize)
	return &Evel{
		baseURL:             baseURL,
		topic:               topic,
		client:              client,
...
func (evel *Evel) PostEvent(evt Event) error {
	...
	fmt.Println("Inside PostEvent.%+v", evt)
	fmt.Println("Inside PostEvent Evel.%+v", evel)
...
func (evel *Evel) doPost(queryPath string, req interface{}) error {
	...
	if err != nil {
		fmt.Println("Error while posting doPost.", err.Error())

		return err
	}
	fmt.Println("Event Posted success......doPost")
